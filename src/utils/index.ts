...
export function utilMediaFileName(mediaUrl: string): string {
...
export function utilVideoFormat(fileName: string) {
 ...
 */
export function utilFloorToDecimals(value: number, decimals = 2) {
...
export function formatDurationToISO8601(duration: number): string {
  return `PT${Math.floor(duration)}S`; // ISO 8601 형식 예: PT218S
}

export function formatProgress(played: number, total: number): number {
  if (total === 0) return +"0.00";
  return utilFloorToDecimals(played / total);
}
